...
static inline void layernorm_backward_2d(Tensor* t) {
    ...
    int used_cuda = 0;
#ifdef USE_CUDA
    used_cuda = cuda_layernorm_2d_backward(x->data, y->grad,
                                           ln->gamma.data,
                                           x->grad, ln->gamma.grad, ln->beta.grad,
                                           N, C, ln->eps);
#endif
...
static inline void layernorm_backward_3d(Tensor* t) {
    ...
    int used_cuda = 0;
#ifdef USE_CUDA
    used_cuda = cuda_layernorm_3d_backward(x->data, y->grad,
                                           ln->gamma.data,
                                           x->grad, ln->gamma.grad, ln->beta.grad,
                                           B, T, C, ln->eps);
#endif
...
static inline void layernorm_forward_2d(LayerNorm* ln, const Tensor* x, Tensor* y) {
    ...
    int used_cuda = 0;
#ifdef USE_CUDA
    used_cuda = cuda_layernorm_2d(x->data, y->data,
                                  ln->gamma.data, ln->beta.data,
                                  N, C, ln->eps);
#endif
...
static inline void layernorm_forward_3d(LayerNorm* ln, const Tensor* x, Tensor* y) {
    ...
    int used_cuda = 0;
#ifdef USE_CUDA
    used_cuda = cuda_layernorm_3d(x->data, y->data,
                                  ln->gamma.data, ln->beta.data,
                                  B, T, C, ln->eps);
#endif
