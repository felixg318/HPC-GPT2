...
static inline void embedding_backward_2d(Tensor* t) {
    ...
    int total = B * T;
    int* d_idx = NULL;
    if (CUDA_CHECK(cudaMallocManaged((void**)&d_idx, total * sizeof(int)))) {
        memcpy(d_idx, idx, total * sizeof(int));
        used_cuda = cuda_embedding_backward_2d(d_idx,
                                               out->grad,
                                               emb->weight.grad,
                                               B, T, emb->num_embeddings, emb->embedding_dim);
