...
#include "cuda_utils.h"  // CUDA_CHECK and stubs when USE_CUDA is off
...
#ifdef USE_CUDA
    // Unified memory lets us avoid explicit H2D/D2H copies for CUDA kernels.
    if (!CUDA_CHECK(cudaMallocManaged((void**)&t->data, n * sizeof(float))) ||
        !CUDA_CHECK(cudaMallocManaged((void**)&t->grad, n * sizeof(float)))) {
        printf("tensor_init: ERROR: cudaMallocManaged failed for %d elements\n", n);
    }
    // Gradients start at zero.
    if (t->grad) {
        CUDA_CHECK(cudaMemset(t->grad, 0, n * sizeof(float)));
    }
#else
