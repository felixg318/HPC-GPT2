...
static inline void linear_backward_2d(Tensor* t) {
    ...
    int used_cuda = 0;
#ifdef USE_CUDA
    // Use matmul backward helpers: grad_x and grad_weight
    used_cuda = cuda_matmul_2d_backward(y->grad,
                                        x->data,
                                        lin->weight.data,
                                        x->grad,
                                        lin->weight.grad,
...
static inline void linear_backward_3d(Tensor* t) {
    ...
    int used_cuda = 0;
#ifdef USE_CUDA
    // Flatten (B,T,C_in) into (N, C_in) to match the matmul_2d_backward contract.
    int N = B * T;
    used_cuda = cuda_matmul_2d_backward(y->grad,
                                        x->data,
                                        lin->weight.data,
...
static inline void linear_forward_2d(Linear* lin, const Tensor* x, Tensor* y) {
    ...
    int used_cuda = 0;
#ifdef USE_CUDA
    used_cuda = cuda_matmul_2d(x->data, lin->weight.data, y->data, N, C_in, C_out);
#endif
    ...
    if (used_cuda) {
        if (lin->use_bias) {
            for (int n = 0; n < N; ++n) {
                for (int o = 0; o < C_out; ++o) {
                    y->data[tensor_index2(y, n, o)] += tensor_get1(&lin->bias, o);
                }
            }
        }
    } else {
...
static inline void linear_forward_3d(Linear* lin, const Tensor* x, Tensor* y) {
    ...
    int used_cuda = 0;
#ifdef USE_CUDA
    used_cuda = cuda_matmul_2d(x->data, lin->weight.data, y->data, B * T, C_in, C_out);
#endif
    ...
    if (used_cuda) {
        if (lin->use_bias) {
            ...
            for (int r = 0; r < rows; ++r) {
                for (int o = 0; o < C_out; ++o) {
                    y->data[r * C_out + o] += tensor_get1(&lin->bias, o);
                }
            }
        }
    } else {
