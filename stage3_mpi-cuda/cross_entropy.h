...
static inline void cross_entropy_backward(Tensor* t) {
    ...
    float inv_token_count = ctx->inv_token_count;
        ...
        for (int b = 0; b < B; ++b) {
            for (int t_ = 0; t_ < T; ++t_) {
                ...
                for (int v = 0; v < V; ++v) {
                    ...
                    logits->grad[tensor_index3(logits, b, t_, v)] += grad * inv_token_count;
                }
            }
        }
    } else {
...
#ifdef USE_CUDA
        if (!cuda_grad_scale(logits->grad, n, inv_token_count)) {
            for (int i = 0; i < n; ++i) {
                logits->grad[i] *= inv_token_count;
            }
        }
#else
        for (int i = 0; i < n; ++i) {
...
#endif
    float denom = (accum[1] > 0.0) ? (float)accum[1] : 1.0f;
    float mean_loss = (float)(accum[0] / (double)denom);
    float inv_token_count = 1.0f / denom;
