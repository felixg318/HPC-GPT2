// checkpoint.h
// Minimal utilities to save/load model parameter tensors.
...
// Write parameters to a binary file: count, then for each tensor -> ndim, shape, numel, data.
...
// Read parameters from a binary file, validating shapes match.
    ...
    for (int i = 0; i < params->count; ++i) {
        ...
        for (int d = 0; d < ndim; ++d) {
            if (shape_file[d] != t->shape[d]) {
                printf("load_weights: shape mismatch for tensor %d dim %d (file=%d expected=%d)\n",
